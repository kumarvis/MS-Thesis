...
def calculate_neurons_and_run(df_images, df_labels):
    """This function calculates the neurons and run neural network for each calculations

    Parameters
    ----------
    df_images : dataframe containing the dataset examples, each row treated as one example.

    df_labels : dataframe containing the dataset examples. each row treated as one example label
...
def neural_network(df_images, df_labels, no_of_neurons):
    ...
    classifier.hidden_layer_sizes = (no_of_neurons, no_of_neurons)
...
def classwise_accuracy(classifier, test_X, test_y, train_X, train_y):
    '''This fn calcualte and prints classwise accuracy for all the classes. It grouby the dataset based on classes then do the calculations'''
...
if __name__=="__main__":
    ...
    df_features = data.drop(['0'], axis=1)
    df_labels = data[['0']]
    calculate_neurons_and_run(df_features, df_labels)
    print('exit')